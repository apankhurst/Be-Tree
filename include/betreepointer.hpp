...
public:
    ...
    void _serialize(std::iostream &fs, serialization_context &context) {
        fs << func << " ";
        serialize(fs, context, params);
    }
    
    void _deserialize(std::iostream &fs, serialization_context &context) {
        fs >> func;
        deserialize(fs, context, &params);
    }
...
private:
    ...
    public:
        ...
        void apply(const MessageKey<Key> &mkey, const Message &elt,
                   Value &default_value) {
            if(elt.func == "insert") {
                ...
                if(bet->function_map.find(elt.func) == bet->function_map.end())
                    assert(0);
                ...
                if (iter == elements.end() || iter->first.key != mkey.key) {
                    if (is_leaf()) {
                        ...
                        apply(mkey, Message("insert", (void *)v),
                              default_value);
                    } else {
                        elements[mkey] = elt;
                    }
                  ...
                  Value *v = new Value;
									*v = bet->function_map[elt.func](*((Value *)iter->second.params), elt.params);
        ...
        Value query(const betree & b, const Key k) const
        {
            ...
            if (message_iter == elements.end() || k < message_iter->first) {
                ...
                v = *((Value *)(message_iter->second.params));
                message_iter++;
            } else {
                ...
                try {
                    Value t = get_pivot(k)->second.child->query(b, k);
                    v = t;
                ...
                while (message_iter != elements.end() && message_iter->first.key == k) {
                    ...
                    v = bet->function_map[message_iter->second.func](v,message_iter->second.params);
                    message_iter++;
                }
            }
            return v;
        ...
        void _deserialize(std::iostream &fs, serialization_context &context) {
            std::string dummy;
            fs >> dummy;
            deserialize(fs, context, (void **)&bet);
            fs >> dummy;
    ...
    static Value __insert(Value &v, void *p) {
    ...
    Value default_value;
    std::unordered_map<std::string, Value (*)(Value &, void *)> function_map ;
...
public:
    ...
    function_map()
    {
        root = ss->allocate(new node(this));
        function_map["insert"] = &__insert;
        function_map["delete"] = nullptr;
    }
    
    void register_function(std::string name, Value (*f)(Value &, void *)) {
    ...
    // occurs.
    void upsert(std::string func, Key k, void *params)
    {
        if(function_map.find(func) == function_map.end())
            return;
        message_map tmp;
        tmp[MessageKey<Key>(k, next_timestamp++)] = Message(func, params);
    ...
    void insert(Key k, void *params){
        upsert("insert", k, params);
    }
    
    void update(Key k, void *params)
    {
    ...
    public:
        ...
        void apply(const MessageKey<Key> &msgkey, const Message &msg) {
            if(bet.function_map.find(msg.func) == bet.function_map.end())
                return;
            if(msg.func == "insert") {
                first = msgkey.key;
